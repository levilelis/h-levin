...
for iter in {1..5}; do
	for weight in 1.0 1.5 2.0 2.5 3.0; do
		for scheme in "${heuristic_scheme[@]}"; do
			...
			sbatch --output=${output_exp} --export=scheme="${scheme}",algorithm=${algorithm},loss=${loss},model=${model},scheduler=${scheduler},weight=${weight} run_bootstrap_train_astar_stp.sh
		done
	done
done
